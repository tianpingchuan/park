...
//	对表单绑定校验
	$('#form_stall_add_edit').validationEngine('attach',{
...
//				执行新增或修改
	...
	$('#button_add').off('click').on('click',function(){
		$('#form_stall_add_edit')[0].reset();
		$('#modal_stall').modal('show');
//		清空所有的校验信息
		$('.formError').remove();
//		尝试移出data-skip的属性（为了唯一性校验）
		$('#stallCode').removeAttr('data-skip');
...
//	绑定修改超链接
	$('#dataTable_wrapper').off('click','#update').on('click','#update',function(){
		...
		$.ajax({
	 		// role/goupdate/1
	 		url : 'stall/goupdate/' + rowId,
	 		success : function(stall) {
//	 			if(true)	js代表true的
	 			if(stall) {
	 	 			$('#modal_stall').modal('show');
//	 	 			清空所有的校验信息
	 	 			$('.formError').remove();
	 	 			var stallCode = stall.stallCode;
//	 	 			在需要进行唯一性校验的filed中加入data-skip属性并赋值
	 	 			$('#stallCode').val(stallCode).attr('data-skip',stallCode);
	 	 			$('#rowId').val(stall.rowId);
	 	 			$('#action_info').html('修改');
	 			}
	 		}
	 	});
...
function saveOrUpdate() {
	...
	if(rowId){//如果主键有信息，则判断是修改
		$.ajax({
			type:'post',
			url:'stall/doupdate',
			data:$('#form_stall_add_edit').serialize(),
			dataType:'json',
			success:function(result){
				if(result){
					$('#modal_stall').modal('hide');
					initTalbeData(1);
				}
			}
		});
